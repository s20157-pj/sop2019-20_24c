...
//Clients'details//
  ...
  fprintf(filelog, "-----------------------------------------------\n");
  fprintf(filelog, "Connection with Client: %s:%s\n", host_, service_);
...
void ConnectedSocketReady(int connectedSocket) {
  ...
  //printf("%s\n", filename);
  fprintf(filelog, "Reqeusted file: \"%s\"\n", filename);
  long fileSize;
  char* bufferFile;
  
 FILE* f = fopen(filename, "r");
	if (f == NULL){
		printf("Could not open/find file(\"%s\")\n", filename);
		fileSize = -1;
		write(connectedSocket, &fileSize, sizeof(fileSize));//zad2
	}
	else{
	fseek(f, 0, SEEK_END);
	fileSize = ftell(f);
	fprintf(filelog, "Filesize: %ld\n", fileSize);
	fseek(f, 0, SEEK_SET);	
